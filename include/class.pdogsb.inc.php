...
{   		
    ...
    private static $bdd='dbname=gsb_appfrais_db';   		
...
*/
    public function getInfosComptable($login_c, $mdp_c)
    {
        $req = "select comptable.id as id, comptable.nom as nom, comptable.prenom as prenom from comptable 
        where comptable.login='$login_c' and comptable.mdp='$mdp_c'";
        $rs = PdoGsb::$monPdo->query($req);
        $ligne = $rs->fetch();
        return $ligne;
    }
...
*/
    ...
    {
        $req = "select * from fichefrais, visiteur where id = idVisiteur and idEtat='VA' ;"; 
        $res = PdoGsb::$monPdo->query($req);
        $laLigne = $res->fetchAll();
        return $laLigne;
    }
// obtenir toutes les informations des personnes qui ont des fiches de frais à l'état VA
